...
function AppContent({ children }: { children: React.ReactNode }) {
  ...
  const calculateAge = (birthday: Date) => {
    ...
    const ageInWeeks = Math.floor((todayTime - birthDate) / (1000 * 60 * 60 * 24 * 7));
    const ageInMonths = Math.floor((todayTime - birthDate) / (1000 * 60 * 60 * 24 * 30.44));
    const ageInYears = Math.floor((todayTime - birthDate) / (1000 * 60 * 60 * 24 * 365.25));
  ...
  const handleUnlock = (caretakerId?: string) => {
    ...
    if (caretakerId) {
      ...
      globalThis.dispatchEvent(caretakerChangedEvent);
    }
  };
  ...
  const handleLogout = async () => {
    ...
    localStorage.removeItem('sidebar-collapsed'); // Clear sidebar state on logout
  ...
  const checkScreenWidth = useCallback(() => {
    if (typeof globalThis !== 'undefined') {
      const isWide = globalThis.innerWidth > 600;
  ...
  useEffect(() => {
    ...
    globalThis.addEventListener('click', updateUnlockTimer);
    globalThis.addEventListener('keydown', updateUnlockTimer);
    globalThis.addEventListener('mousemove', updateUnlockTimer);
    globalThis.addEventListener('touchstart', updateUnlockTimer);

    // Add resize listener
    globalThis.addEventListener('resize', checkScreenWidth);

    return () => {
      // Clean up event listeners
      globalThis.removeEventListener('click', updateUnlockTimer);
      globalThis.removeEventListener('keydown', updateUnlockTimer);
      globalThis.removeEventListener('mousemove', updateUnlockTimer);
      globalThis.removeEventListener('touchstart', updateUnlockTimer);
      globalThis.removeEventListener('resize', checkScreenWidth);
    };
  ...
  useEffect(() => {
    ...
    const checkAuthStatus = () => {
      ...
      if (unlockTime) {
        const lastActivity = Number.parseInt(unlockTime);
  ...
  useEffect(() => {
    ...
    globalThis.addEventListener('openPaymentModal', handleOpenPayment);
    return () => globalThis.removeEventListener('openPaymentModal', handleOpenPayment);
  }, []);
    ...
    <>
      {shouldShowAppUI && (
        <div className="min-h-screen flex">
          ...
          {/* Main content area */}
          <div className={`flex flex-col flex-1 min-h-screen transition-all duration-300 ${isWideScreen ? (isCollapsed ? 'w-[calc(100%-4rem)]' : 'w-[calc(100%-16rem)]') : 'w-full'}`}>
...
export default function AppLayout({
  ...
  const handleLogout = async () => {
    ...
    if (isAccountAuth) {
      ...
      const familySlug = globalThis.location.pathname.split('/')[1];
      if (familySlug) {
        globalThis.location.href = `/${familySlug}`;
      } else {
